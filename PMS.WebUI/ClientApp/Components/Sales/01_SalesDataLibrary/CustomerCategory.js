...
(function (CustomerCategory) {
    ...
    var Model = new P_D_SalesCustomerCategory();
    ...
    function Assign() {
        ...
        for (var i = 0; i < CountGrid; i++) {
            ...
            if (StatusFlag == "u") {
                var UpdatedDetail = dataSource.filter(function (x) { return x.CustomerCategoryID == $("#txt_ID" + i).val(); });
    ...
    function AddNewRow() {
        if (!SharedSession.CurrentPrivileges.AddNew)
            return;
        var CanAdd = true;
        if (CountGrid > 0) {
            for (var i = 0; i < CountGrid; i++) {
