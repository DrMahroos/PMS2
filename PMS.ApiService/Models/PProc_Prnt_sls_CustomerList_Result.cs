//------------------------------------------------------------------------------
// <auto-generated>
//     This code was generated from a template.
//
//     Manual changes to this file may cause unexpected behavior in your application.
//     Manual changes to this file will be overwritten if the code is regenerated.
// </auto-generated>
//------------------------------------------------------------------------------

namespace PMS.ApiService.Models
{
    using System;
    
    public partial class PProc_Prnt_sls_CustomerList_Result
    {
        public Nullable<int> Comp { get; set; }
        public string CompNameA { get; set; }
        public string CompNameE { get; set; }
        public string BraNameA { get; set; }
        public string BraNameE { get; set; }
        public string LoginUser { get; set; }
        public Nullable<System.DateTime> PrintDate { get; set; }
        public string Par_active { get; set; }
        public string Par_Temp { get; set; }
        public string Par_branchnameA { get; set; }
        public string Par_branchnameE { get; set; }
        public string Par_CatDescA { get; set; }
        public string Par_CatDescE { get; set; }
        public string Par_EngDescA { get; set; }
        public string Par_EngDescE { get; set; }
        public Nullable<int> Par_FromCust { get; set; }
        public Nullable<int> Par_ToCust { get; set; }
        public Nullable<int> CustomerCode { get; set; }
        public Nullable<bool> IsTemporary { get; set; }
        public string DescA { get; set; }
        public string DescE { get; set; }
        public string ContactPerson { get; set; }
        public string ContactTel { get; set; }
        public string Tel1 { get; set; }
        public string Tel2 { get; set; }
        public string Fax { get; set; }
        public string Mobile { get; set; }
        public string Email { get; set; }
        public Nullable<bool> IsActive { get; set; }
        public string CrNo { get; set; }
        public string ChamberNo { get; set; }
        public string VatNo { get; set; }
        public Nullable<int> BraCode { get; set; }
        public string EngCode { get; set; }
        public string eng_DescA { get; set; }
        public string eng_DescE { get; set; }
        public string CustomerCatCode { get; set; }
        public string cat_DescA { get; set; }
        public string CatDescE { get; set; }
        public int Projects { get; set; }
        public int AllOffers { get; set; }
    }
}
