...
{
    public class P_D_LaborAbsenceTypeController : BaseController, IBaseController
    {
        [HttpPost]
        public ResponseResult DeleteEntity(JObject data)
        {
            ...
            var obj = JObjectData<P_D_LaborAbsenceType>(data);
            obj.Entity = result.ResponseData as P_D_LaborAbsenceType;
            data = JObjectToObj<P_D_LaborAbsenceType>(obj.Entity, obj.Session);
            result = this.Execute<P_D_LaborAbsenceType>(data, ActionTypes.Delete, this.Process);
            return result;
        }
        ...
        public ResponseResult InsertEntity(JObject data)
        {
            ...
            using (var dbTransaction = db.Database.BeginTransaction())
            {
                try
                {
                    ...
                    obj.Entity = result.ResponseData as P_D_LaborAbsenceType;
                    data = JObjectToObj<P_D_LaborAbsenceType>(obj.Entity, obj.Session);
                    result = this.Execute<P_D_LaborAbsenceType>(data, ActionTypes.Insert, this.Process, db, dbTransaction);
                    SessionRecord session = obj.Session;

                    if (result.ResponseState)
                    {
                        var Abs = JsonDeserialize<P_D_LaborAbsenceType>(result.ResponseData.ToString());
                        ResponseResult res = Shared.InvTransactionProcess(session, Abs.LaborAbsenceTypeID, "Abs", db);
                        if (res.ResponseState == true)
                        {

                            dbTransaction.Commit();
                            Abs.AbsCode = res.ResponseData.ToString();
        ...
        {
            var result = this.Get<P_D_LaborAbsenceType>(SqlStatement);
        ...
        public ResponseResult UpdateEntity(JObject data)
        {
            ...
            using (var dbTransaction = db.Database.BeginTransaction())
            {
                try
                {
                    ...
                    obj.Entity = result.ResponseData as P_D_LaborAbsenceType;
                    data = JObjectToObj<P_D_LaborAbsenceType>(obj.Entity, obj.Session);
                    result = this.Execute<P_D_LaborAbsenceType>(data, ActionTypes.Update, this.Process, db, dbTransaction);
                    SessionRecord session = obj.Session;
