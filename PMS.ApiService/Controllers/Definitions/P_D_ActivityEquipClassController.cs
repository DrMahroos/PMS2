...
{
    public class P_D_ActivityEquipClassController : BaseController, IBaseController
    {
        [HttpPost]
        public ResponseResult DeleteEntity(JObject data)
        {
            ...
            var obj = JObjectData<P_D_ActivityEquipClass>(data);
            obj.Entity = result.ResponseData as P_D_ActivityEquipClass;
            data = JObjectToObj<P_D_ActivityEquipClass>(obj.Entity, obj.Session);
            result = this.Execute<P_D_ActivityEquipClass>(data, ActionTypes.Delete, this.Process);
            return result;
        }
        ...
        public ResponseResult InsertEntity(JObject data)
        {
            ...
            using (var dbTransaction = db.Database.BeginTransaction())
            {
                try
                {
                    ...
                    obj.Entity = result.ResponseData as P_D_ActivityEquipClass;
                    data = JObjectToObj<P_D_ActivityEquipClass>(obj.Entity, obj.Session);
                    result = this.Execute<P_D_ActivityEquipClass>(data, ActionTypes.Insert, this.Process, db, dbTransaction);
                    SessionRecord session = obj.Session;

                    if (result.ResponseState)
                    {
                        var SEC = JsonDeserialize<P_D_ActivityEquipClass>(result.ResponseData.ToString());
                        ResponseResult res = Shared.InvTransactionProcess(session, SEC.ActivityEquipClassID, "ActEquip", db);
        ...
        {
            var result = this.Get<P_D_ActivityEquipClass>(SqlStatement);
        ...
        public ResponseResult UpdateEntity(JObject data)
        {
            ...
            using (var dbTransaction = db.Database.BeginTransaction())
            {
                try
                {
                    ...
                    obj.Entity = result.ResponseData as P_D_ActivityEquipClass;
                    data = JObjectToObj<P_D_ActivityEquipClass>(obj.Entity, obj.Session);
                    result = this.Execute<P_D_ActivityEquipClass>(data, ActionTypes.Update, this.Process, db, dbTransaction);
                    SessionRecord session = obj.Session;
