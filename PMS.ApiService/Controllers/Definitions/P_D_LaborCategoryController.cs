...
{
    public class P_D_LaborCategoryController : BaseController, IBaseController
    {
        [HttpGet]
        public object SelectEntity(string SqlStatement)
        {
            var result = this.Get<P_D_LaborCategory>(SqlStatement);
        ...
        public ResponseResult DeleteEntity(JObject data)
        {
            ...
            var obj = JObjectData<P_D_LaborCategory>(data);
            obj.Entity = result.ResponseData as P_D_LaborCategory;
            data = JObjectToObj<P_D_LaborCategory>(obj.Entity, obj.Session);
            result = this.Execute<P_D_LaborCategory>(data, ActionTypes.Delete, this.Process);
            return result;
        }
        ...
        public ResponseResult InsertEntityOld(JObject data)
        {
            ResponseResult result = new Models.ResponseResult();
            using (var dbTransaction = db.Database.BeginTransaction())
            {
                try
                {
                    ...
                    obj.Entity = result.ResponseData as P_D_LaborCategory;
                    data = JObjectToObj<P_D_LaborCategory>(obj.Entity, obj.Session);
                    result = this.Execute<P_D_LaborCategory>(data, ActionTypes.Insert, this.Process, db, dbTransaction);
                    SessionRecord session = obj.Session;

                    if (result.ResponseState)
                    {
                        var SEC = JsonDeserialize<P_D_LaborCategory>(result.ResponseData.ToString());
                        ResponseResult res = Shared.InvTransactionProcess(session, SEC.LaborCategoryId, "lCat", db);
                        if (res.ResponseState == true)
                        {

                            dbTransaction.Commit();
                            SEC.CategCode = res.ResponseData.ToString();
        ...
        public ResponseResult InsertEntity(JObject data)
        {
            ...
            using (var dbTransaction = db.Database.BeginTransaction())
            {
                try
                {
                    ...
                    if (result.ResponseState)
                    {
                        var Eq = JsonDeserialize<P_D_LaborCategory>(result.ResponseData.ToString());
                        ResponseResult res = Shared.InvTransactionProcess(session, Eq.LaborCategoryId, "lCat", db);
                        if (res.ResponseState == true)
                        {

                            dbTransaction.Commit();
                            Eq.CategCode = res.ResponseData.ToString();
        ...
        public ResponseResult UpdateEntity(JObject data)
        {
            ...
            using (var dbTransaction = db.Database.BeginTransaction())
            {
                try
                {
                    ...
                    obj.Entity = result.ResponseData as P_D_LaborCategory;
                    data = JObjectToObj<P_D_LaborCategory>(obj.Entity, obj.Session);
                    result = this.Execute<P_D_LaborCategory>(data, ActionTypes.Update, this.Process, db, dbTransaction);
                    SessionRecord session = obj.Session;
