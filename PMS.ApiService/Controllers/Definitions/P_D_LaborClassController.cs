...
{
    public class P_D_LaborClassController : BaseController, IBaseController
    {
        [HttpPost]
        public ResponseResult DeleteEntity(JObject data)
        {
            ...
            var obj = JObjectData<P_D_LaborClass>(data);
            obj.Entity = result.ResponseData as P_D_LaborClass;
            data = JObjectToObj<P_D_LaborClass>(obj.Entity, obj.Session);
            result = this.Execute<P_D_LaborClass>(data, ActionTypes.Delete, this.Process);
            return result;
        }
        ...
        public ResponseResult InsertEntity(JObject data)
        {
            ...
            using (var dbTransaction = db.Database.BeginTransaction())
            {
                try
                {
                    ...
                    obj.Entity = result.ResponseData as P_D_LaborClass;
                    data = JObjectToObj<P_D_LaborClass>(obj.Entity, obj.Session);
                    result = this.Execute<P_D_LaborClass>(data, ActionTypes.Insert, this.Process, db, dbTransaction);
                    SessionRecord session = obj.Session;

                    if (result.ResponseState)
                    {
                        var SEC = JsonDeserialize<P_D_LaborClass>(result.ResponseData.ToString());
                        ResponseResult res = Shared.InvTransactionProcess(session, SEC.LaborClassId, "lCls", db);
                        if (res.ResponseState == true)
                        {

                            dbTransaction.Commit();
                            SEC.ClassCode = res.ResponseData.ToString();
        ...
        {
            var result = this.Get<P_D_LaborClass>(SqlStatement);
        ...
        public ResponseResult UpdateEntity(JObject data)
        {
            ...
            using (var dbTransaction = db.Database.BeginTransaction())
            {
                try
                {
                    ...
                    obj.Entity = result.ResponseData as P_D_LaborClass;
                    data = JObjectToObj<P_D_LaborClass>(obj.Entity, obj.Session);
                    result = this.Execute<P_D_LaborClass>(data, ActionTypes.Update, this.Process, db, dbTransaction);
                    SessionRecord session = obj.Session;
                    ...
                    var model = from serv in db.P_D_SiteEngCategory
                                where
                                    ...
                                    (
                                        ...
                                        serv.CategCode == entity.ClassCode
                                    )
                                select serv;
                    if (model.Count() > 0)
                    {
