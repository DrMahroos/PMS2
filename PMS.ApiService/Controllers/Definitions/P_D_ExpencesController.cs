...
{
    public class P_D_ExpencesController : BaseController, IBaseController
    {
        [HttpPost]
        public ResponseResult DeleteEntity(JObject data)
        {
            ...
            var obj = JObjectData<P_D_Expences>(data);
            obj.Entity = result.ResponseData as P_D_Expences;
            data = JObjectToObj<P_D_Expences>(obj.Entity, obj.Session);
            result = this.Execute<P_D_Expences>(data, ActionTypes.Delete, this.Process);
            return result;
        }
        ...
        public ResponseResult InsertEntity(JObject data)
        {
            ...
            using (var dbTransaction = db.Database.BeginTransaction())
            {
                try
                {
                    ...
                    obj.Entity = result.ResponseData as P_D_Expences;
                    data = JObjectToObj<P_D_Expences>(obj.Entity, obj.Session);
                    result = this.Execute<P_D_Expences>(data, ActionTypes.Insert, this.Process, db, dbTransaction);
                    SessionRecord session = obj.Session;

                    if (result.ResponseState)
                    {
                        var Exp = JsonDeserialize<P_D_Expences>(result.ResponseData.ToString());
                        ResponseResult res = Shared.InvTransactionProcess(session, Exp.ExpencesID, "Expences", db);
                        if (res.ResponseState == true)
                        {

                            dbTransaction.Commit();
                            Exp.ExpCode = res.ResponseData.ToString();
                            result.ResponseData = JsonSerialize(Exp);
                        }
                        else
                        {
        ...
        {
            var result = this.Get<P_D_Expences>(SqlStatement);
        ...
        public ResponseResult UpdateEntity(JObject data)
        {
            ...
            using (var dbTransaction = db.Database.BeginTransaction())
            {
                try
                {
                    ...
                    obj.Entity = result.ResponseData as P_D_Expences;
                    data = JObjectToObj<P_D_Expences>(obj.Entity, obj.Session);
                    result = this.Execute<P_D_Expences>(data, ActionTypes.Update, this.Process, db, dbTransaction);
                    SessionRecord session = obj.Session;
