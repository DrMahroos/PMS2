...
{
    public class P_D_ExpencesCategoryController : BaseController, IBaseController
    {
        [HttpPost]
        public ResponseResult DeleteEntity(JObject data)
        {
            ...
            var obj = JObjectData<P_D_ExpencesCategory>(data);
            obj.Entity = result.ResponseData as P_D_ExpencesCategory;
            data = JObjectToObj<P_D_ExpencesCategory>(obj.Entity, obj.Session);
            result = this.Execute<P_D_ExpencesCategory>(data, ActionTypes.Delete, this.Process);
            return result;
        }
        ...
        public ResponseResult InsertEntity(JObject data)
        {
            ...
            using (var dbTransaction = db.Database.BeginTransaction())
            {
                try
                {
                    ...
                    obj.Entity = result.ResponseData as P_D_ExpencesCategory;
                    data = JObjectToObj<P_D_ExpencesCategory>(obj.Entity, obj.Session);
                    result = this.Execute<P_D_ExpencesCategory>(data, ActionTypes.Insert, this.Process, db, dbTransaction);
                    SessionRecord session = obj.Session;

                    if (result.ResponseState)
                    {
                        var Ex = JsonDeserialize<P_D_ExpencesCategory>(result.ResponseData.ToString());
                        ResponseResult res = Shared.InvTransactionProcess(session, Ex.ExpCatID, "Expcat", db);
                        if (res.ResponseState == true)
                        {

                            dbTransaction.Commit();
                            Ex.ExpCatCode = res.ResponseData.ToString();
                            result.ResponseData = JsonSerialize(Ex);
                        }
                        else
                        {
        ...
        {
            var result = this.Get<P_D_ExpencesCategory>(SqlStatement);
        ...
        public ResponseResult UpdateEntity(JObject data)
        {
            ...
            using (var dbTransaction = db.Database.BeginTransaction())
            {
                try
                {
                    ...
                    obj.Entity = result.ResponseData as P_D_ExpencesCategory;
                    data = JObjectToObj<P_D_ExpencesCategory>(obj.Entity, obj.Session);
                    result = this.Execute<P_D_ExpencesCategory>(data, ActionTypes.Update, this.Process, db, dbTransaction);
                    SessionRecord session = obj.Session;
