...
{
    public class P_D_ScopeCategoryController : BaseController, IBaseController
    {
        [HttpPost]
        public ResponseResult DeleteEntity(JObject data)
        {
            ...
            var obj = JObjectData<P_D_ScopeCategory>(data);
            obj.Entity = result.ResponseData as P_D_ScopeCategory;
            data = JObjectToObj<P_D_ScopeCategory>(obj.Entity, obj.Session);
            result = this.Execute<P_D_ScopeCategory>(data, ActionTypes.Delete, this.Process);
            return result;
        }
        ...
        public ResponseResult InsertEntity(JObject data)
        {
            ...
            using (var dbTransaction = db.Database.BeginTransaction())
            {
                try
                {
                    ...
                    obj.Entity = result.ResponseData as P_D_ScopeCategory;
                    data = JObjectToObj<P_D_ScopeCategory>(obj.Entity, obj.Session);
                    result = this.Execute<P_D_ScopeCategory>(data, ActionTypes.Insert, this.Process, db, dbTransaction);
                    SessionRecord session = obj.Session;

                    if (result.ResponseState)
                    {
                        var Scop = JsonDeserialize<P_D_ScopeCategory>(result.ResponseData.ToString());
                        ResponseResult res = Shared.InvTransactionProcess(session, Scop.ScopeCategoryID, "Scop", db);
                        if (res.ResponseState == true)
                        {

                            dbTransaction.Commit();
                            Scop.ScopeCategCode = res.ResponseData.ToString();
                            result.ResponseData = JsonSerialize(Scop);
                        }
                        else
                        {
        ...
        {
            var result = this.Get<P_D_ScopeCategory>(SqlStatement);
        ...
        public ResponseResult UpdateEntity(JObject data)
        {
            ...
            using (var dbTransaction = db.Database.BeginTransaction())
            {
                try
                {
                    ...
                    obj.Entity = result.ResponseData as P_D_ScopeCategory;
                    data = JObjectToObj<P_D_ScopeCategory>(obj.Entity, obj.Session);
                    result = this.Execute<P_D_ScopeCategory>(data, ActionTypes.Update, this.Process, db, dbTransaction);
                    SessionRecord session = obj.Session;
