...
{
    public class P_D_UnProdCategoryController : BaseController, IBaseController
    {
        [HttpPost]
        public ResponseResult DeleteEntity(JObject data)
        {
            ...
            var obj = JObjectData<P_D_UnProdCategory>(data);
            obj.Entity = result.ResponseData as P_D_UnProdCategory;
            data = JObjectToObj<P_D_UnProdCategory>(obj.Entity, obj.Session);
            result = this.Execute<P_D_UnProdCategory>(data, ActionTypes.Delete, this.Process);
            return result;
        }
        ...
        public ResponseResult InsertEntity(JObject data)
        {
            ...
            using (var dbTransaction = db.Database.BeginTransaction())
            {
                try
                {
                    ...
                    obj.Entity = result.ResponseData as P_D_UnProdCategory;
                    data = JObjectToObj<P_D_UnProdCategory>(obj.Entity, obj.Session);
                    result = this.Execute<P_D_UnProdCategory>(data, ActionTypes.Insert, this.Process, db, dbTransaction);
                    SessionRecord session = obj.Session;

                    if (result.ResponseState)
                    {
                        var Abs = JsonDeserialize<P_D_UnProdCategory>(result.ResponseData.ToString());
                        ResponseResult res = Shared.InvTransactionProcess(session, Abs.UnProdCategoryID, "Prod", db);
                        if (res.ResponseState == true)
                        {

                            dbTransaction.Commit();
                            Abs.CategCode = res.ResponseData.ToString();
        ...
        {
            var result = this.Get<P_D_UnProdCategory>(SqlStatement);
        ...
        public ResponseResult UpdateEntity(JObject data)
        {
            ...
            using (var dbTransaction = db.Database.BeginTransaction())
            {
                try
                {
                    ...
                    obj.Entity = result.ResponseData as P_D_UnProdCategory;
                    data = JObjectToObj<P_D_UnProdCategory>(obj.Entity, obj.Session);
                    result = this.Execute<P_D_UnProdCategory>(data, ActionTypes.Update, this.Process, db, dbTransaction);
                    SessionRecord session = obj.Session;
