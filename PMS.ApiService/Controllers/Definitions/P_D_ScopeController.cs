...
{
    public class P_D_ScopeController : BaseController, IBaseController
    {
        [HttpPost]
        public ResponseResult DeleteEntity(JObject data)
        {
            ...
            var obj = JObjectData<P_D_Scope>(data);
            obj.Entity = result.ResponseData as P_D_Scope;
            data = JObjectToObj<P_D_Scope>(obj.Entity, obj.Session);
            result = this.Execute<P_D_Scope>(data, ActionTypes.Delete, this.Process);
            return result;
        }
        ...
        public ResponseResult InsertEntity(JObject data)
        {
            ...
            using (var dbTransaction = db.Database.BeginTransaction())
            {
                try
                {
                    ...
                    obj.Entity = result.ResponseData as P_D_Scope;
                    data = JObjectToObj<P_D_Scope>(obj.Entity, obj.Session);
                    result = this.Execute<P_D_Scope>(data, ActionTypes.Insert, this.Process, db, dbTransaction);
                    SessionRecord session = obj.Session;

                    if (result.ResponseState)
                    {
                        var Scop = JsonDeserialize<P_D_Scope>(result.ResponseData.ToString());
                        ResponseResult res = Shared.InvTransactionProcess(session, Scop.ScopeID, "ScopDef", db);
                        if (res.ResponseState == true)
                        {

                            dbTransaction.Commit();
                            Scop.ScopeCode = res.ResponseData.ToString();
        ...
        {
            var result = this.Get<P_D_Scope>(SqlStatement);
        ...
        public ResponseResult UpdateEntity(JObject data)
        {
            ...
            using (var dbTransaction = db.Database.BeginTransaction())
            {
                try
                {
                    ...
                    obj.Entity = result.ResponseData as P_D_Scope;
                    data = JObjectToObj<P_D_Scope>(obj.Entity, obj.Session);
                    result = this.Execute<P_D_Scope>(data, ActionTypes.Update, this.Process, db, dbTransaction);
                    SessionRecord session = obj.Session;
