...
{
    public class P_D_LaborOverTimeTypeController : BaseController, IBaseController
    {
        [HttpPost]
        public ResponseResult DeleteEntity(JObject data)
        {
            ...
            var obj = JObjectData<P_D_LaborOverTimeType>(data);
            obj.Entity = result.ResponseData as P_D_LaborOverTimeType;
            data = JObjectToObj<P_D_LaborOverTimeType>(obj.Entity, obj.Session);
            result = this.Execute<P_D_LaborOverTimeType>(data, ActionTypes.Delete, this.Process);
            return result;
        }
        ...
        public ResponseResult InsertEntity(JObject data)
        {
            ...
            using (var dbTransaction = db.Database.BeginTransaction())
            {
                try
                {
                    ...
                    obj.Entity = result.ResponseData as P_D_LaborOverTimeType;
                    data = JObjectToObj<P_D_LaborOverTimeType>(obj.Entity, obj.Session);
                    result = this.Execute<P_D_LaborOverTimeType>(data, ActionTypes.Insert, this.Process, db, dbTransaction);
                    SessionRecord session = obj.Session;

                    if (result.ResponseState)
                    {
                        var Abs = JsonDeserialize<P_D_LaborOverTimeType>(result.ResponseData.ToString());
                        ResponseResult res = Shared.InvTransactionProcess(session, Abs.LaborOverTimeTypeID, "Over", db);
                        if (res.ResponseState == true)
                        {

                            dbTransaction.Commit();
                            Abs.OverTimeCode = res.ResponseData.ToString();
        ...
        {
            var result = this.Get<P_D_LaborOverTimeType>(SqlStatement);
        ...
        public ResponseResult UpdateEntity(JObject data)
        {
            ...
            using (var dbTransaction = db.Database.BeginTransaction())
            {
                try
                {
                    ...
                    obj.Entity = result.ResponseData as P_D_LaborOverTimeType;
                    data = JObjectToObj<P_D_LaborOverTimeType>(obj.Entity, obj.Session);
                    result = this.Execute<P_D_LaborOverTimeType>(data, ActionTypes.Update, this.Process, db, dbTransaction);
                    SessionRecord session = obj.Session;
