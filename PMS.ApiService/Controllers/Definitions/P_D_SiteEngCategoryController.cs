...
{
    public class P_D_SiteEngCategoryController : BaseController, IBaseController
    {
        [HttpGet]
        public object SelectEntity(string SqlStatement)
        {
            var result = this.Get<P_D_SiteEngCategory>(SqlStatement);
        ...
        public ResponseResult InsertEntity(JObject data)
        {
            ...
            using (var dbTransaction = db.Database.BeginTransaction())
            {
                try
                {
                    ...
                    var obj = JObjectData<P_D_SiteEngCategory>(data);
                    obj.Entity = result.ResponseData as P_D_SiteEngCategory;
                    data = JObjectToObj<P_D_SiteEngCategory>(obj.Entity, obj.Session);
                    result = this.Execute<P_D_SiteEngCategory>(data, ActionTypes.Insert, this.Process, db, dbTransaction);
                    SessionRecord session = obj.Session;

                    if (result.ResponseState)
                    {
                        var SEC = JsonDeserialize<P_D_SiteEngCategory>(result.ResponseData.ToString());
                        ResponseResult res = Shared.InvTransactionProcess(session, SEC.SiteEngCategoryId, "SEC", db);
        ...
        public ResponseResult UpdateEntity(JObject data)
        {
            ...
            using (var dbTransaction = db.Database.BeginTransaction())
            {
                try
                {
                    ...
                    obj.Entity = result.ResponseData as P_D_SiteEngCategory;
                    data = JObjectToObj<P_D_SiteEngCategory>(obj.Entity, obj.Session);
                    result = this.Execute<P_D_SiteEngCategory>(data, ActionTypes.Update, this.Process, db, dbTransaction);
                    SessionRecord session = obj.Session;
        ...
        public ResponseResult DeleteEntity(JObject data)
        {
            ...
            obj.Entity = result.ResponseData as P_D_SiteEngCategory;
            data = JObjectToObj<P_D_SiteEngCategory>(obj.Entity, obj.Session);
            result = this.Execute<P_D_SiteEngCategory>(data, ActionTypes.Delete, this.Process);
            return result;
        ...
        public ResponseResult InsertCollection(JObject data)
        {
            ...
            obj.Entity = result.ResponseData as P_D_SiteEngCategory;
            data = JObjectToObj<P_D_SiteEngCategory>(obj.Entity, obj.Session);
            result = this.Execute<P_D_SiteEngCategory>(data, ActionTypes.InsertCollection, this.Process);
            return result;
        }
        ...
        public ResponseResult Validation(ActionTypes actionType, JObject data)
        {
            ...
            if (String.IsNullOrEmpty(entity.CategCode))
            {
                result.ResponseState = false;
                    ...
                    var model = from serv in db.P_D_SiteEngCategory
                                where
                                    ...
                                    (
                                        ...
                                        serv.CategCode == entity.CategCode
                                    )
                                select serv;
                    if (model.Count() > 0)
                    {
