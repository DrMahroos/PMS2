...
{
    public class P_D_EquipmentClassController : BaseController, IBaseController
    {
        /// <summary>
        ...
        {
            var result = this.Get<P_D_EquipmentClass>(SqlStatement);
        ...
        public ResponseResult InsertEntity(JObject data)
        {
            ...
            using (var dbTransaction = db.Database.BeginTransaction())
            {
                try
                {
                    ...
                    var obj = JObjectData<P_D_EquipmentClass>(data);
                    obj.Entity = result.ResponseData as P_D_EquipmentClass;
                    data = JObjectToObj<P_D_EquipmentClass>(obj.Entity, obj.Session);
                    result = this.Execute<P_D_EquipmentClass>(data, ActionTypes.Insert, this.Process, db, dbTransaction);
                    SessionRecord session = obj.Session;

                    if (result.ResponseState)
                    {
                        var Eq = JsonDeserialize<P_D_EquipmentClass>(result.ResponseData.ToString());
                        ResponseResult res = Shared.InvTransactionProcess(session, Eq.EquipClassId, "Equ", db);
                        if (res.ResponseState == true)
                        {

                            dbTransaction.Commit();
                            Eq.ClassCode = res.ResponseData.ToString();
        ...
        public ResponseResult UpdateEntity(JObject data)
        {
            ...
            using (var dbTransaction = db.Database.BeginTransaction())
            {
                try
                {
                    ...
                    obj.Entity = result.ResponseData as P_D_EquipmentClass;
                    data = JObjectToObj<P_D_EquipmentClass>(obj.Entity, obj.Session);
                    result = this.Execute<P_D_EquipmentClass>(data, ActionTypes.Update, this.Process, db, dbTransaction);
                    SessionRecord session = obj.Session;
        ...
        public ResponseResult DeleteEntity(JObject data)
        {
            ...
            obj.Entity = result.ResponseData as P_D_EquipmentClass;
            data = JObjectToObj<P_D_EquipmentClass>(obj.Entity, obj.Session);
            result = this.Execute<P_D_EquipmentClass>(data, ActionTypes.Delete, this.Process);
            return result;
        ...
        public ResponseResult InsertCollection(JObject data)
        {
            ...
            obj.Entity = result.ResponseData as P_D_EquipmentClass;
            data = JObjectToObj<P_D_EquipmentClass>(obj.Entity, obj.Session);
            result = this.Execute<P_D_EquipmentClass>(data, ActionTypes.InsertCollection, this.Process);
            return result;
        }
        ...
        public ResponseResult Validation(ActionTypes actionType, JObject data)
        {
            ...
            if (String.IsNullOrEmpty(entity.ClassCode))
            {
                result.ResponseState = false;
            ...
            switch (actionType)
            {
                case ActionTypes.Insert:
                    var model = from serv in db.P_D_EquipmentClass
                                where
                                    serv.CompCode == entity.CompCode &&
                                    (
                                        serv.DescA == entity.DescA ||
                                        serv.DescE == entity.DescE ||
                                        serv.ClassCode == entity.ClassCode
                                    )
                                select serv;
                    if (model.Count() > 0)
                    {
