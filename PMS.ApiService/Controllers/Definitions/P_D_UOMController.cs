...
{
    public class P_D_UOMController : BaseController, IBaseController
    {
        [HttpPost]
        public ResponseResult DeleteEntity(JObject data)
        {
            ...
            var obj = JObjectData<P_D_UOM>(data);
            obj.Entity = result.ResponseData as P_D_UOM;
            data = JObjectToObj<P_D_UOM>(obj.Entity, obj.Session);
            result = this.Execute<P_D_UOM>(data, ActionTypes.Delete, this.Process);
            return result;
        }
        ...
        public ResponseResult InsertEntity(JObject data)
        {
            ...
            using (var dbTransaction = db.Database.BeginTransaction())
            {
                try
                {
                    ...
                    obj.Entity = result.ResponseData as P_D_UOM;
                    data = JObjectToObj<P_D_UOM>(obj.Entity, obj.Session);
                    result = this.Execute<P_D_UOM>(data, ActionTypes.Insert, this.Process, db, dbTransaction);
                    SessionRecord session = obj.Session;

                    if (result.ResponseState)
                    {
                        var uom = JsonDeserialize<P_D_UOM>(result.ResponseData.ToString());
                        ResponseResult res = Shared.InvTransactionProcess(session, uom.UomID, "Uom", db);
                        if (res.ResponseState == true)
                        {

                            dbTransaction.Commit();
                            uom.UomCode = res.ResponseData.ToString();
                            result.ResponseData = JsonSerialize(uom);
                        }
                        else
                        {
        ...
        {
            var result = this.Get<P_D_UOM>(SqlStatement);
        ...
        public ResponseResult UpdateEntity(JObject data)
        {
            ...
            using (var dbTransaction = db.Database.BeginTransaction())
            {
                try
                {
                    ...
                    obj.Entity = result.ResponseData as P_D_UOM;
                    data = JObjectToObj<P_D_UOM>(obj.Entity, obj.Session);
                    result = this.Execute<P_D_UOM>(data, ActionTypes.Update, this.Process, db, dbTransaction);
                    SessionRecord session = obj.Session;
