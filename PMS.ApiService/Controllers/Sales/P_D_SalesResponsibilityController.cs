...
{
    public class P_D_SalesResponsibilityController : BaseController, IBaseController
    {
        [HttpPost]
        public ResponseResult DeleteEntity(JObject data)
        {
            ...
            var obj = JObjectData<P_D_SalesResponsibility>(data);
            obj.Entity = result.ResponseData as P_D_SalesResponsibility;
            data = JObjectToObj<P_D_SalesResponsibility>(obj.Entity, obj.Session);
            result = this.Execute<P_D_SalesResponsibility>(data, ActionTypes.Delete, this.Process);
            return result;
        }
        ...
        public ResponseResult InsertEntity(JObject data)
        {
            ...
            using (var dbTransaction = db.Database.BeginTransaction())
            {
                try
                {
                    ...
                    obj.Entity = result.ResponseData as P_D_SalesResponsibility;
                    data = JObjectToObj<P_D_SalesResponsibility>(obj.Entity, obj.Session);
                    result = this.Execute<P_D_SalesResponsibility>(data, ActionTypes.Insert, this.Process, db, dbTransaction);
                    SessionRecord session = obj.Session;

                    if (result.ResponseState)
                    {
                        var SEC = JsonDeserialize<P_D_SalesResponsibility>(result.ResponseData.ToString());
                        ResponseResult res = Shared.InvTransactionProcess(session, SEC.ReposibilityId, "SLSResp", db);
                        if (res.ResponseState == true)
                        {

                            dbTransaction.Commit();
                            SEC.ResposibilityCode = res.ResponseData.ToString();
        ...
        {
            var result = this.Get<P_D_SalesResponsibility>(SqlStatement);
        ...
        public ResponseResult UpdateEntity(JObject data)
        {
            ...
            using (var dbTransaction = db.Database.BeginTransaction())
            {
                try
                {
                    ...
                    obj.Entity = result.ResponseData as P_D_SalesResponsibility;
                    data = JObjectToObj<P_D_SalesResponsibility>(obj.Entity, obj.Session);
                    result = this.Execute<P_D_SalesResponsibility>(data, ActionTypes.Update, this.Process, db, dbTransaction);
                    SessionRecord session = obj.Session;
        ...
        public ResponseResult Validation(ActionTypes actionType, JObject data)
        {
            ...
            if (String.IsNullOrEmpty(entity.ResposibilityCode))
            {
                result.ResponseState = false;
                return result;
            }
